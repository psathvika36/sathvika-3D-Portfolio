    ...
    if (name && email && subject && message) {
      ...
      if (emailValid) {
        ...
        fetch('https://api.emailjs.com/api/v1.0/email/send', {
            ...
            body: JSON.stringify({
              user_id: 'xhkXTaRTB0sZkhLY9', // Replace with your EmailJS user ID
              service_id: 'service_bovtki6', // Replace with your EmailJS service ID
              template_id: 'template_l54sspf', // Replace with your EmailJS template ID
              template_params: {
                  'name': name,
                  'email': email,
                  'subject': subject,
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const butterfly = document.querySelector(".animated-butterfly");
  const animeGirl = document.querySelector(".animated-anime-girl");
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        animeGirl.style.animationPlayState = "running";
      } else {
        butterfly.style.animationPlayState = "paused";
        animeGirl.style.animationPlayState = "paused";
      }
    });
  };
